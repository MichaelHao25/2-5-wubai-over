...
function loade(slide_obj, refresh_obj) {
	...
	function slide_Effect() {
		...
		function move(even) { //手指移动
			if (evene == 1) {
				if (down == 1) {
					var difference = even.clientY - default_value;
				}
			} else {
			...
			if ($(window).scrollTop() == 0 && difference > 0) {
				...
				$(slide_obj).css({
					"-webkit-transform": "translate3d(0," + difference / 2.4 + "px,0)",
...
function slide_menu(main_div, click_hide, click_show, bettow) { //左侧划出菜单
	...
	if (wibody > objwid) {
		...
		$(main_div).removeClass("opacity").css({
			"margin-left": bettow * wibody
		});
		$(click_show).click(function() {
			$(main_div).animate({
				"margin-left": 0
...
function alertbox(click_show, click_hide, obj, box_vague) { //弹窗js
	...
	$(click_show).click(function() {
		$(window).on("touchmove", function(even) {
			even.preventDefault()
		});
		$(obj).fadeIn();
		$(obj).find(".layout").css({
...
mybody = document.getElementById('public-index')
mybody.style.height = h + 'px';
// ddd
...
mybody.addEventListener('touchstart', function(ev) {
	...
	// alert(ev.path[0]);
	// if (!$(ev.path[0]).hasClass('no-preventDefault')) {
	// }
	// alert($(ev.path[0]).hasClass('no-preventDefault'));
	ev.preventDefault()
	// if ($(ev.path[0]).hasClass('no-preventDefault')) {
	// 	$('.goto-luck-draw').trigger('click');
	// }
...
$(".home span").on("touchstart", function(even) {
	showbox = true;
	var m = $(this).index();
...
$('.btm a').click(function() {
	//$('.mobile-wrap').removeClass('show');

})

$('.gift .close').on('touchstart', function() {
	$(this).parent().removeAttr('style');
	$('.' + $(this).parent().attr('next-page')).css('top', 0);
});

function luck_draw_init() {
	if (window.DeviceMotionEvent) {
		window.addEventListener('devicemotion', deviceMotionHandler, false);
		document.getElementsByClassName('luck-draw')[0].addEventListener('touchstart', deviceMotionHandler, false);
	}
	...
	function deviceMotionHandler(eventData) {
		...
		try {
			...
			if (animateEnd) {
				...
				setTimeout(function() {
					$('.luck-draw-sign').removeClass('active');
					$('.luck-draw').addClass('switch');
					setTimeout(function() {
						$('.luck-draw').removeClass('show');
						// rand(3, 1)
						switch (rand(3, 1)) {
							case 1:
								$('.center.mobile-wrap.win').addClass('show');
								break;
							case 2:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico24');
								break;
							case 3:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico25');
								break;
							case 4:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico26');
								break;
							case 5:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico27');
								break;
							case 6:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico28');
								break;
							case 7:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico29');
								break;
							case 8:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico30');
								break;
							case 9:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico31');
								break;
							case 10:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico32');
								break;
							case 11:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico33');
								break;
							case 12:
								$('.mobile-wrap.center.no-win-bless').addClass('show open-hico34');
								break;
							default:
								$('.mobile-wrap.center.no-win-trop').addClass('show');
						}
					}, 2500)
					animateEnd = false
				}, 4500)
...
$('.goto-luck-draw').on('touchstart', function() {
	$('.mobile-wrap.center.index').removeClass('show');
	$('#public-index').hide();
	$('.luck-draw').addClass('show');
	luck_draw_init();
});



$('.open-share').on('touchstart', function() {
	$('.no-win-bless-title.share').addClass('show');
});
